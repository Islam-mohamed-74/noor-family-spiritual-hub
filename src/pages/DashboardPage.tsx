...
import { BookOpen, Moon, Sun, EyeOff, Flame } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className={kidsMode ? "text-xl" : "text-lg"}>
            🕌 الصلوات اليومية
          </CardTitle>
        </CardHeader>
            ...
            >
              ...
              {prayer.completed && (
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      updatePrayer(prayer.name, "onTime", !prayer.onTime)
                    }
                  ...
                  <button
                    onClick={() =>
                      updatePrayer(prayer.name, "jamaah", !prayer.jamaah)
                    }
        ...
        <CardContent className="flex gap-4">
          <div className="flex-1 flex items-center justify-between p-3 rounded-lg bg-secondary">
            <span className="flex items-center gap-2">
              <Sun className="h-4 w-4" /> أذكار الصباح
            </span>
            <Switch
              checked={log.azpiMorning}
              onCheckedChange={(v) => {
                updateLog({ azpiMorning: v });
                if (v) showEncouragement("azkar");
              }}
            />
          </div>
          <div className="flex-1 flex items-center justify-between p-3 rounded-lg bg-secondary">
            <span className="flex items-center gap-2">
              <Moon className="h-4 w-4" /> أذكار المساء
            </span>
            <Switch
              checked={log.azpiEvening}
              onCheckedChange={(v) => {
                updateLog({ azpiEvening: v });
                if (v) showEncouragement("azkar");
              }}
            />
          </div>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={log.fasting}
                onCheckedChange={(v) => {
                  updateLog({ fasting: v, fastingType: v ? "fard" : undefined });
                  if (v) showEncouragement("fasting");
                }}
              />
            </div>
            {log.fasting && (
      ...
      <Card className="border-dashed">
        <CardHeader className="pb-3">
          ...
          <p className="text-xs text-muted-foreground">
            هذه العبادات مخفية عن الآخرين وتكسب نقاطاً
          </p>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between">
            <span>صدقة</span>
            <Switch
              checked={log.sadaqaPrivate}
              onCheckedChange={(v) => updateLog({ sadaqaPrivate: v })}
            />
          </div>
          <div className="flex items-center justify-between">
            <span>دعاء خاص</span>
            <Switch
              checked={log.duaPrivate}
              onCheckedChange={(v) => updateLog({ duaPrivate: v })}
            />
          </div>
        </CardContent>
      </Card>
        ...
        <Card className="border-gold/50 bg-gradient-to-b from-gold/5 to-transparent">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={log.iftar}
                onCheckedChange={(v) => updateLog({ iftar: v })}
              />
            </div>
            <div className="flex items-center justify-between">
              <span>التراويح</span>
              <Switch
                checked={log.tarawih}
                onCheckedChange={(v) => updateLog({ tarawih: v })}
              />
            </div>
            {log.tarawih && (
              <div className="flex items-center gap-3">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateLog({ tarawihRakaat: parseInt(e.target.value) || 0 })
                  }
